...
- Test pages must stay separated by patient and writer and unavailable to model tuning. Patient/encounter grouping comes from ingestion; writer identity is optional and recorded by the annotator from the visible signature, as an opaque identifier and never a clinician's name (D-024). Writer coverage will be partial and must be reported, not assumed.
