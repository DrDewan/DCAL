...
const state = {
  ...
  queue: null,
  task: null,
  annotator: localStorage.getItem("dcal.annotator") || "",
...
async function uploadFiles(fileList) {
  ...
  $("#upload-progress-text").textContent = `Preparing ${files.length} source${files.length === 1 ? "" : "s"}…`;
  try {
    const result = await api("/api/uploads", { method: "POST", body: form });
    const created = result.tasks.filter((item) => item.created).length;
    const reused = result.tasks.length - created;
    toast(`${created} page${created === 1 ? "" : "s"} added${reused ? `; ${reused} duplicate${reused === 1 ? "" : "s"} reused` : ""}.`);
    await loadQueue();
    if (result.tasks.length === 1) await openTask(result.tasks[0].id);
  } catch (error) {
...
function ensureIdentity() {
  $("#identity-label").textContent = state.annotator || "Set annotator";
  if (!state.annotator && !$("#identity-dialog").open) $("#identity-dialog").showModal();
}
...
async function saveTask(status = null) {
  ...
  if (!state.annotator) { ensureIdentity(); throw new Error("Set an annotator name before saving."); }
  state.saving = true;
  ...
  try {
    const task = await api(`/api/tasks/${state.task.id}`, {
      ...
      body: JSON.stringify({ annotation: state.task.annotation, expected_version: state.task.version, actor: state.annotator, status }),
    });
...
function bindApp() {
  ...
  $("#identity-button").addEventListener("click", () => { $("#identity-input").value = state.annotator; $("#identity-dialog").showModal(); });
  $("#identity-form").addEventListener("submit", (event) => {
    const value = $("#identity-input").value.trim();
    if (!value) { event.preventDefault(); return; }
    state.annotator = value; localStorage.setItem("dcal.annotator", value); $("#identity-label").textContent = value;
  });
...
async function init() {
  ...
  try {
    ...
    if (id && state.annotator) await openTask(id);
  } catch (error) {
    toast(error.message, "error");
  }
}
